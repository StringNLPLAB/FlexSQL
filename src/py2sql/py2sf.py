...
from src.utils import divide, compare_pandas_table, now, diff_outputs, get_subquery_schema, get_question_str
from pathlib import Path

DIR = os.path.dirname(__file__)
MAX_RETRIES = 3

DB_PATH = os.path.join(DIR, "../../datasets/Spider2")
DB_TYPE = "snowflake"
SUBQUERY_TYPE_PATH = os.path.join(DIR, "../../datasets/Spider2/spider2-snow/resource/databases_no_nulls_2")
...
def py2sql(agent: Chat, qid: str, question: dict, plan: str,
           ...
           logger: logging.Logger = None, db_type: str = "snowflake", db_path: str = None, **kwargs) -> bool:
...
3. It uses all identifiers consistently since quoted identifiers in Snowflake are case-sensitive.

Translation Preliminaries:
1. If the Python program’s behavior depends on runtime types (e.g., isinstance(x, str), dict, list, tuple), the SQL translation must preserve that behavior. For instance, do not use string operations unless the runtime value is a string.
2. The SQL translation can handle (semi-)structured data types (e.g., Snowflake's VARIANT and nested objects/arrays). When a Python code snippet defines a helper function (regardless of its name) that:
...
- converts a JSON array or dictionary into a Python list
- is applied row-wise via Series.apply, map, or equivalent
then this code snippet is unnesting a Snowflake VARIANT column. It should be translated into `(LATERAL) FLATTEN(input => COLUMN | PARSE_JSON(COLUMN))`
3. The SQL translation clearly distinguishes quoted identifiers and strings/regular expressions.
4. Note that Python and Snowflake use different regex semantics. When translating `str.contains(REGEX, case=bool)` to SQL, use `REGEXP_LIKE(str, '.*' || REGEX || '.*', flags)` in Snowflake. You MUST prepend and append ".*" to the REGEX to ensure semantics equivalence between Python and Snowflake. If `case=False`, you MUST use the flags 'is' in REGEXP_LIKE to enable case-insensitive. Do NOT use 'i' alone. The flag 'is' is required to ensure semantic consistence with Python’s regex behavior.
5. When writing SQL queries that access VARIANT data types in Snowflake (columns where each value is a JSON object), you must specify the full access path including nested keys and array indices.
- **Case-Sensitive Keys:** Use single quotes for exact matches: "column_name":"KeyName" (always use this for key names to ensure case sensitivity).
- **Nested Object Access:** "column_name":"OuterKey":"InnerKey".
- **Array Element Access:** You must use zero-based indexing to access array elements. Don't skip this step if applicable: Example: "column_name"[0].
...
6. For columns where value is a string that has the structure of a JSON object:
- You should first use LOAD_JSON() function to load the string into a JSON object, then use the access rules above to access the keys.
""".strip()
    code = """
```python
{program}
...
""".strip().format(program=program)
    ...
    def translate_program(prompt, sql_file, sql_out_file):
        try:
            ...
            for turn in range(MAX_RETRIES):
                ...
                # print("Usage: python program_frame.py <cred_json> <question_id> <program_path> <queries_path> <output_csv>")
                command = [sys.executable, "utils/sql_frame.py", "snowflake",
                           os.path.join(db_path, "spider2-snow/evaluation_suite/snowflake_credential.json"), qid,
                           sql_file, sql_out_file]
    ...
    def gen_sql_query(sql_file, sql_out_file):
        ...
        if kwargs.get("external_knowledge", None) is not None:
            prompt_chunks = prompt.split("### Python program:")
            prompt = '\n'.join([prompt_chunks[0].strip(), f'\n### Some other useful knowledge:\n{kwargs["external_knowledge"]}', "\n### Python program:", prompt_chunks[1].lstrip()])
    ...
    def repair_sql(code, py_diff, sql_file, sql_out_file, incorrect_sql, sql_diff):
        ...
        atomic_subqueries = list(subqueries.values())
        repair_prompt = f"""
### Your Task:
You are given:
* A user question
...
4. It uses all identifiers consistently since quoted identifiers in Snowflake are case-sensitive.
...
""".strip()
        if kwargs.get("external_knowledge", None) is not None:
            prompt_chunks = repair_prompt.split("### Python program:")
            repair_prompt = '\n'.join([prompt_chunks[0].strip(), f'\n### Some other useful knowledge:\n{kwargs["external_knowledge"]}', "\n### Python program:", prompt_chunks[1].lstrip()])
        logger.info(f"<prompt>\n{repair_prompt}\n</prompt>")
        # Define example structure for validation
...
def main(args):
    ...
    def read_translate_tasks(args):
        # read questions
        question_file = os.path.join(DB_PATH, "spider2-snow/spider2-snow.jsonl")
        nl_questions = {}
        ext_knowledges = {}
        with open(question_file, 'r') as reader:
            for line in reader:
                ...
                if line.get("external_knowledge", None) is not None:
                    ext_knowledge_file = os.path.join(DB_PATH, "spider2-snow/resource/documents", line["external_knowledge"])
                    with open(ext_knowledge_file, 'r') as ext_reader:
                        ext_knowledges[line["instance_id"]] = ext_reader.read()
        ...
        with open(major_voting_file, 'r') as reader:
            ...
            for qid, group_info in groups.items():
                ...
                with open(plan_file, 'r') as reader:
                    plans = ['\n'.join(plan[0]) for plan in json.load(reader)]

                gen_sql_files = []  # SQL queries generated from frontend LLMs
                sql_files, sql_out_files, py_files, py_out_files, qid_plans = [], [], [], [], []
                ...
                if len(gen_sql_files) == 0:
                    ...
                    # read atomic SQL sub-queries
                    subquery_file = os.path.join(args.input_experiment_dir, f"{qid}/sub_queries_plan_{plan_id}.json")
                    with open(subquery_file, 'r') as reader:
                        subqueries = json.load(reader)

                    # read atomic SQL sub-queries
                    subqueries_schema = {}
                    for db, subquery in subqueries.items():
                        subquery_type_file = os.path.join(SUBQUERY_TYPE_PATH, db.replace('.', os.path.sep) + '.json')
                        subqueries_schema[db] = get_subquery_schema(qid, subquery, subquery_type_file)
                    ...
                    qid_tasks = []
                    for sql_file, sql_out_file, py_file, py_out_file, plan in zip(sql_files, sql_out_files, py_files, py_out_files, qid_plans):
                        # read program
                        ...
                        qid_tasks.append({
                            ...
                            "subqueries": subqueries, "subqueries_schema": subqueries_schema,
                            "external_knowledge": ext_knowledges.get(qid, None),
